import pytest
from ctdna_sentinel import VAFKineticsTrackerAgent, MRDStatusClassifierAgent, ClonalEvolutionDetectorAgent, LiquidBiopsyCoordinator, main


def test_sub_agents():
    a1 = VAFKineticsTrackerAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = MRDStatusClassifierAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = ClonalEvolutionDetectorAgent()
...
def test_domain_registry():
    from ctdna_sentinel import DomainKnowledgeRegistry
